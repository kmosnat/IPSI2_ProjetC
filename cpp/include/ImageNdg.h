...
class CImageNdg {
	...
	///////////////////////////////////////
		...
		_declspec(dllexport) CImageNdg difference(const CImageNdg& img) const;

		// morphologie
		_declspec(dllexport) CImageNdg morphologie(const std::string& methode = "erosion", const std::string& eltStructurant = "V8", int taille = 3); // choix "erosion" / "dilatation", "V8"/"V4"

		_declspec(dllexport) CImageNdg ouverture(const std::string& strel = "V4", int taille = 3);
		_declspec(dllexport) CImageNdg fermeture(const std::string& strel = "V4", int taille = 3);
